...
function findMangaById(id) {
  return state.catalog.find(function(m) { return String(m.id) === String(id); });
}
...
function resumeManga(mangaId, chapterNum) {
  const manga = findMangaById(mangaId);
  if (manga) {
...
async function showDetail(id) {
  ...
  for (var s = 5; s >= 1; s--) {
    ...
    starsHtml += '<span class="star-rating-btn' + isStarActive + '" onclick="submitMangaRating(' + safeId + ', ' + s + ')">★</span>';
  }
  ...
  if (state.bookmarks[manga.id]) {
    ...
    actionClick = isAnime ? 'openVideoPlayer(' + safeId + ', ' + book.chapter + ')' : 'openReader(' + book.chapter + ')';
  }
      ...
      '<div class="detail-header">' +
        ...
        '<div class="detail-info">' +
          ...
          '<div class="detail-actions">' +
            ...
            '<button class="btn-classic-grey" onclick="openLibraryModal(' + safeId + ')">' + (libSection ? '★ Cambiar Estado' : '☆ Agregar a Biblioteca') + '</button>' +
            '<button class="btn-classic-grey" onclick="startDownloadSimulation()">↓ Descargar</button>' +
            (isAdmin() ? '<button class="btn-classic-grey btn-admin-delete" style="background-color:#e74c3c; color:#fff; border-color:#c0392b;" onclick="deleteManga(' + safeId + ')">Eliminar Obra (Admin)</button>' : '') +
          '</div>' +
        '</div>' +
      '</div>' +
      ...
      '<div class="comments-section">' +
        ...
        '<div class="comment-form"><strong>Deja tu comentario:</strong>' +
          '<div class="comment-input-area">' +
            ...
            '<button class="btn-comentar" onclick="addComment(' + safeId + ')">Enviar</button>' +
          '</div>' +
        '</div>' +
...
async function submitMangaRating(mangaId, stars) {
  var manga = findMangaById(mangaId);
  if (!manga) return;
  var currentRating = parseFloat(manga.rating || 5.0);
  var newRating = ((currentRating * 9) + parseFloat(stars)) / 10;
...
function deleteLocalManga(mangaId) {
  state.catalog = state.catalog.filter(function(m) { return String(m.id) !== String(mangaId); });
...
function openUploadChapterModal() {
  ...
  if (fileInput) {
    fileInput.accept = isAnime ? 'video;
